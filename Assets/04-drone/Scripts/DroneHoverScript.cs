...
namespace _04_drone.Scripts
{
    public class DroneHoverScript : Agent
    {
        ...
        public override void OnActionReceived(ActionBuffers actionBuffers)
        {
            ...
            float actionFrontRight = Mathf.Clamp(actionBuffers.ContinuousActions[0], -1f, 1f) * upForce; // One of many actions
            float actionFrontLeft = Mathf.Clamp(actionBuffers.ContinuousActions[1], -1f, 1f) * upForce;
            float actionBackRight = Mathf.Clamp(actionBuffers.ContinuousActions[2], -1f, 1f) * upForce;
            float actionBackLeft = Mathf.Clamp(actionBuffers.ContinuousActions[3], -1f, 1f) * upForce;
            
            Debug.Log("actions = " + actionFrontRight + ", " + actionFrontLeft + ", " + actionBackRight + ", " + actionBackLeft);
            ...
            // markerRBody.AddForceAtPosition(marker.transform.up * actionFrontRight, frontRightThruster.transform.position);
            // markerRBody.AddForceAtPosition(marker.transform.up * actionFrontLeft, frontLeftThruster.transform.position);
            // markerRBody.AddForceAtPosition(marker.transform.up * actionBackRight, backRightThruster.transform.position);
            // markerRBody.AddForceAtPosition(marker.transform.up * actionBackLeft, backLeftThruster.transform.position);
            markerRBody.AddForce(marker.transform.up * actionFrontRight);
            markerRBody.AddForce(marker.transform.up * actionFrontLeft);
            markerRBody.AddForce(marker.transform.up * actionBackRight);
            markerRBody.AddForce(marker.transform.up * actionBackLeft);
        ...
        // Tell the ML algorithm everything you can about the current state
        public float NewMyCalculateReward(float energyUsed)
        {
